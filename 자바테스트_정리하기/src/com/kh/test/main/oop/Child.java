...
//메서드
	//생성자 기본 ctrl space 이긴 하지만 constructor 표시가 있는지 확인후 엔터
	public Child() {
		
	}
	//만약에 super 를 기본생성자와 필수생성자 불러오고 싶다면
	//			자식생성자도 두개 만들어서 가각 넣어줘야함
	...
	public Child(int 부모변수, int 자식변수) {
		super(부모변수); //Parent에서 필수생성자를 사용하곘다는 의미
		this.자식변수 = 자식변수;
	}

	//생성자 필수 alt shift s o //부모변수는 Parent에서 가져올 예정
	//public Child(/*int 부모변수,*/ int 자식변수) {
		//super(); 
		//부모변수에 어덯게 받아서 가져가겠다 라는 것이 없기 때문에 뜨는 에러
		//부모변수에도 기본 생성자를 넣어서 작성 this.자식변수 = 자식변수;}
